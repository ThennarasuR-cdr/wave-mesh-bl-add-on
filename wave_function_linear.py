...
straight_road_90_piece = Piece('Straight',straight_road_module,(0,0,1,1),90)
...
modular_pieces=[straight_road_piece,empty,l_turn_road_piece,r_turn_road_piece,cross_road_piece,t_road_piece,l_turn_270_road_piece,r_turn_90_road_piece,t_road_90_piece,t_road_180_piece,t_road_270_piece,straight_road_90_piece]
    ...
    if piece!=None:
        for possible_piece in cell.possibilities:
            if possible_piece.x_top == piece.x_bottom:
                print("1, x_top, x_bottom", possible_piece.x_top,piece.x_bottom,possible_piece.name, piece.name)
    ...
    if piece!=None:
        for possible_piece in cell.possibilities:
            if possible_piece.x_bottom == piece.x_top:
                print(f"2, {x-1}-{y}, x_bottom, x_top", possible_piece.x_bottom,piece.x_top,possible_piece.name, piece.name)
                updated_possibilities.append(possible_piece)
    if len(updated_possibilities) != 0:
        cell.possibilities = updated_possibilities
    ...
    if piece!=None:
        for possible_piece in cell.possibilities:
            if possible_piece.y_top == piece.y_bottom:
                print(f"3, {x}-{y+1}, y_top, y_bottom", possible_piece.y_top,piece.y_bottom,possible_piece.name, piece.name)
                updated_possibilities.append(possible_piece)
    if len(updated_possibilities) != 0:
        cell.possibilities = updated_possibilities
    ...
    if piece!=None:
        for possible_piece in cell.possibilities:
            if possible_piece.y_bottom == piece.y_top:
                print(f"4, {x}-{y-1}, y_bottom, y_top", possible_piece.y_bottom,piece.y_top,possible_piece.name, piece.name)
                updated_possibilities.append(possible_piece)
    if len(updated_possibilities) != 0:
        cell.possibilities = updated_possibilities
